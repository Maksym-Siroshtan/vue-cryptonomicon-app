    ...
    <div class="container">
      ...
      <section>
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="flex bg-white shadow-md rounded-md flex-wrap">
              <span
                v-for="coin in coinsFilter()"
                @click="add(coin)"
                :key="coin"
      ...
      <template v-if="tickers.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div
            v-for="tiker in paginatedTickers"
            :key="tiker.name"
            @click="select(tiker)"
            :class="{
              'border-4': selectedTicker === tiker,
            }"
...
export default {
  ...
  methods: {
    subscribeToUpdates(tickerName) {
      setInterval(async () => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&api_key=3fd020c799e74da6b71ae33e88aca04867d111f27f77c037327158024b40d354`
        );
    ...
    coinsFilter() {
      return Object.entries(this.coins)
        ...
        .filter((t) =>
          this.ticker === ""
            ? this.ticker
            : t.toLowerCase().indexOf(this.ticker.toLowerCase()) > -1
        )
        .slice(0, 4);
    },
  },
